package net.swindxg.tutorialmod.item.custom;
...
import net.swindxg.tutorialmod.block.ModBlocks;
...
public class ChiselItem extends Item {
    private static final Map<Block, Block> CHISEL_MAP =
            Map.of(
                    ...
                    Blocks.NETHERRACK, ModBlocks.BISMUTH_BLOCK.get()//自己新增的物品要打get()
            );
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if(CHISEL_MAP.containsKey(clickBlock)) {
            if(!level.isClientSide()) {
                level.setBlockAndUpdate(context.getClickedPos(), CHISEL_MAP.get(clickBlock).defaultBlockState());
                //當右鍵金磚 CHISEL_MAP.get(clickBlock)回傳鐵磚 defaultBlockState放置為預設狀態
